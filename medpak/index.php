...
// Pre-compute learner URL for the ctrl view's share widget
if ($isCtrl && $id) {
    $actual_link = (empty($_SERVER['HTTPS']) ? 'http' : 'https') . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
    $base = explode('?', $actual_link)[0];
    $simUrl = $base . "?view=sim&id=" . $id;
...
<body class="bg-slate-900 text-slate-200 font-sans min-h-screen flex flex-col items-center">
    ...
    <?php elseif ($isLinks): ?>
        ...
        <div class="flex-1 flex flex-col items-center justify-center p-6 w-full max-w-4xl">
            <div class="bg-slate-800 border-2 border-slate-700 p-8 rounded-2xl w-full shadow-2xl">
                ...
                <div class="mb-8">
                    <label class="block text-sm font-bold text-slate-400 mb-2 uppercase tracking-wider">Learner Interface</label>
                    <div class="flex gap-2 mb-2">
                        <input type="text" id="sim-url" readonly value="<?= htmlspecialchars($simUrl) ?>" class="flex-1 min-w-0 bg-slate-900 text-cyan-400 font-mono border border-slate-600 rounded-lg p-3 outline-none" />
                        <button onclick="shareUrl('<?= htmlspecialchars($simUrl, ENT_QUOTES) ?>', 'Learner Interface', 'sim')" class="flex-shrink-0 bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 px-4 rounded-lg transition-colors flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg>
                            Share
                        </button>
                    </div>
                    <a href="<?= htmlspecialchars($simUrl) ?>" target="_blank" class="block w-full text-center bg-cyan-700 hover:bg-cyan-600 text-white font-bold py-3 px-4 rounded-lg transition-colors">Open Learner Interface</a>
                </div>

                <div>
                    <label class="block text-sm font-bold text-slate-400 mb-2 uppercase tracking-wider">Admin Interface</label>
                    <div class="flex gap-2 mb-2">
                        <input type="text" id="ctrl-url" readonly value="<?= htmlspecialchars($ctrlUrl) ?>" class="flex-1 min-w-0 bg-slate-900 text-indigo-400 font-mono border border-slate-600 rounded-lg p-3 outline-none" />
                        <button onclick="shareUrl('<?= htmlspecialchars($ctrlUrl, ENT_QUOTES) ?>', 'Admin Interface', 'ctrl')" class="flex-shrink-0 bg-slate-700 hover:bg-slate-600 text-white font-bold py-3 px-4 rounded-lg transition-colors flex items-center gap-1.5">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"></path></svg>
                            Share
                        </button>
                    </div>
                    <a href="<?= htmlspecialchars($ctrlUrl) ?>" target="_blank" class="block w-full text-center bg-indigo-600 hover:bg-indigo-500 text-white font-bold py-3 px-4 rounded-lg transition-colors mb-4">Open Admin Interface</a>
                    <div class="flex items-center gap-4">
                        <div class="flex-shrink-0 bg-white p-2 rounded-lg">
                            <img src="https://api.qrserver.com/v1/create-qr-code/?size=176x176&data=<?= urlencode($ctrlUrl) ?>"
                                 class="w-44 h-44 block" alt="QR Code for Admin Interface" />
                        </div>
                        <p class="text-slate-400 text-sm leading-relaxed">Scan QR code to open Admin Interface on device</p>
                    </div>
                </div>

                <!-- Share toast notification -->
                <div id="share-toast" class="hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-700 text-white text-sm font-semibold px-5 py-3 rounded-xl shadow-xl border border-slate-500 z-50 flex items-center gap-2">
                    ...
                    <span id="share-toast-msg">Link copied!</span>
                </div>
                ...
                function shareUrl(url, label, type) {
                    const body = label + ':\n' + url;
                    if (navigator.share) {
                        navigator.share({ title: 'Telemetry Simulator — ' + label, text: body, url: url })
                            .catch(() => {});
                        return;
                    }
                    // Fallback: show share options
                    const mailLink = 'mailto:?subject=' + encodeURIComponent('Telemetry Simulator — ' + label) + '&body=' + encodeURIComponent(body);
                    const smsLink  = 'sms:?&body=' + encodeURIComponent(body);
                    const menu = document.getElementById('share-menu-' + type);
                    if (menu) {
                        menu.classList.toggle('hidden');
                        return;
                    }
                    // Build inline menu first time
                    const btn = event.currentTarget;
                    const wrapper = document.createElement('div');
                    wrapper.id = 'share-menu-' + type;
                    wrapper.className = 'absolute z-50 mt-1 bg-slate-800 border border-slate-600 rounded-xl shadow-2xl overflow-hidden text-sm font-semibold';
                    wrapper.style.minWidth = '200px';
                    wrapper.innerHTML =
                        '<a href="' + mailLink + '" class="flex items-center gap-2 px-4 py-3 hover:bg-slate-700 text-white transition-colors">' +
                            '<svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>' +
                            'Send via Email</a>' +
                        '<a href="' + smsLink + '" class="flex items-center gap-2 px-4 py-3 hover:bg-slate-700 text-white transition-colors">' +
                            ...
                            'Send via Text</a>' +
                        '<button onclick="navigator.clipboard.writeText(\'' + url.replace(/'/g, "\\'") + '\').then(()=>showToast(\'Link copied to clipboard!\')); document.getElementById(\'share-menu-' + type + '\').classList.add(\'hidden\')" class="w-full flex items-center gap-2 px-4 py-3 hover:bg-slate-700 text-white transition-colors">' +
                    ...
                    setTimeout(() => {
                        document.addEventListener('click', function close(ev) {
                            if (!wrapper.contains(ev.target) && ev.target !== btn) {
    ...
    <?php else: ?>
        ...
        <div <?= $isSim ? 'id="sim-view-wrapper"' : '' ?> class="w-full p-2 md:p-6 flex flex-col items-center">
            ...
            <?php if ($isCtrl && !empty($simUrl)): ?>
            <!-- Share toast + shareUrl function (ctrl view) -->
            <div id="share-toast" class="hidden fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-700 text-white text-sm font-semibold px-5 py-3 rounded-xl shadow-xl border border-slate-500 z-50 flex items-center gap-2">
                <svg class="w-4 h-4 text-green-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                <span id="share-toast-msg">Link copied!</span>
            </div>
            ...
            function showToast(msg) {
                const toast = document.getElementById('share-toast');
                document.getElementById('share-toast-msg').textContent = msg;
                toast.classList.remove('hidden');
                clearTimeout(window._toastTimer);
                window._toastTimer = setTimeout(() => toast.classList.add('hidden'), 2800);
            }
            function shareUrl(url, label, type) {
                const body = label + ':\n' + url;
                if (navigator.share) {
                    navigator.share({ title: 'Telemetry Simulator — ' + label, text: body, url: url }).catch(() => {});
                    return;
                }
                const mailLink = 'mailto:?subject=' + encodeURIComponent('Telemetry Simulator — ' + label) + '&body=' + encodeURIComponent(body);
                const smsLink  = 'sms:?&body=' + encodeURIComponent(body);
                const existing = document.getElementById('share-menu-' + type);
                ...
                wrapper.style.minWidth = '200px';
                wrapper.innerHTML =
                    '<a href="' + mailLink + '" class="flex items-center gap-2 px-4 py-3 hover:bg-slate-700 text-white transition-colors"><svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>Send via Email</a>' +
                    '<a href="' + smsLink + '" class="flex items-center gap-2 px-4 py-3 hover:bg-slate-700 text-white transition-colors"><svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"></path></svg>Send via Text</a>' +
                    '<button onclick="navigator.clipboard.writeText(\'' + url.replace(/'/g, "\\'") + '\').then(()=>showToast(\'Link copied to clipboard!\')); document.getElementById(\'share-menu-' + type + '\').classList.add(\'hidden\')" class="w-full flex items-center gap-2 px-4 py-3 hover:bg-slate-700 text-white transition-colors"><svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 16H6a2 2 0 01-2-2V6a2 2 0 012-2h8a2 2 0 012 2v2m-6 12h8a2 2 0 002-2v-8a2 2 0 00-2-2h-8a2 2 0 00-2 2v8a2 2 0 002 2z"></path></svg>Copy to Clipboard</button>';
                const anchor = btn.parentElement;
                anchor.style.position = 'relative';
                anchor.appendChild(wrapper);
                setTimeout(() => {
            ...
            <div id="admin-panel" class="w-full max-w-6xl bg-slate-800 rounded-xl p-6 shadow-xl border border-slate-700" <?= $isSim ? 'style="display: none;"' : '' ?>>
                ...
                <!-- Defib/Pacer Accordion -->
                <div class="mt-4 border border-slate-700 rounded-lg overflow-hidden">
                    <button onclick="document.getElementById('defibpacer-panel').classList.toggle('hidden'); document.getElementById('defibpacer-chevron').classList.toggle('rotate-180')" class="w-full flex items-center justify-between px-4 py-3 bg-slate-900/50 hover:bg-slate-900/80 text-left transition-colors">
                        <span class="text-sm font-semibold text-slate-300 flex items-center gap-2">
                        ...
                        <svg id="defibpacer-chevron" class="w-4 h-4 text-slate-400 transition-transform duration-200" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                    <div id="defibpacer-panel" class="hidden border-t border-slate-700 p-4 flex flex-col md:flex-row gap-4">

                        <!-- Defib sub-panel -->
                        <div class="flex-1 bg-red-950/30 border border-red-800 rounded-lg p-4 flex flex-col sm:flex-row items-center gap-4">
                            <div class="flex-1 w-full">
                                <label class="block text-sm font-bold text-red-400 mb-2 uppercase tracking-wider">Shock Energy: <span id="lbl-shockEnergy">150</span> J</label>
                                <input type="range" id="input-shockEnergy" min="0" max="8" step="1" value="5" class="w-full accent-red-500 cursor-pointer" />
                            </div>
                            <div class="flex flex-col gap-2 w-full sm:w-auto flex-shrink-0">
                                <div class="flex gap-2">
                                    <button id="btn-sync" class="flex-1 bg-[#CF9C00] text-black font-bold rounded-xl py-2 px-4 border-2 border-[#CF9C00] text-sm uppercase tracking-widest transition-all">SYNC</button>
                                    <button id="btn-ctrl-vol" class="bg-green-600 text-green-100 rounded-xl py-2 px-3 border-2 border-green-500 transition-colors flex items-center justify-center">
                                        <svg id="ctrl-vol-on" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.536 8.464a5 5 0 010 7.072M12 6v12l-4-4H4V10h4l4-4z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 5.343a9 9 0 010 13.314"/>
                                        </svg>
                                        <svg id="ctrl-vol-off" class="w-5 h-5 hidden" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z"/>
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2"/>
                                        </svg>
                                    </button>
                                </div>
                                <button id="btn-ctrl-charge" class="relative w-full overflow-hidden bg-slate-700 hover:bg-slate-600 text-white font-black rounded-xl py-3 px-8 border-2 border-slate-500 text-lg uppercase tracking-wide transition-colors">
                                    <div id="ctrl-charge-fill" class="absolute inset-y-0 left-0 bg-orange-400/60 pointer-events-none" style="width:0%"></div>
                                    <span class="relative">CHARGE</span>
                                </button>
                                <button id="btn-shock" class="w-full bg-red-600 text-white font-black rounded-xl py-4 px-8 shadow-lg shadow-red-900/40 border-2 border-red-400 flex items-center justify-center gap-2 text-xl uppercase transition-all opacity-40 cursor-not-allowed" disabled>
                                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                                    SHOCK
                                </button>
                            </div>
                        ...
                        <!-- Pacer sub-panel -->
                        <div class="flex-1 bg-blue-950/30 border border-blue-800 rounded-lg p-4 flex flex-col sm:flex-row items-center gap-4">
                            <button id="btn-pacing" class="w-full sm:w-auto flex-shrink-0 bg-slate-700 text-slate-300 font-black rounded-xl py-4 px-8 border-2 border-slate-600 text-xl uppercase tracking-wide transition-all">
                                PACING OFF
                            </button>
                            <div class="flex-1 w-full grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                ...
                const markExistingQRSBeeped = () => { events.forEach(e => { if (e.type === 'QRS') e.beeped = true; }); };
                const playBeep = () => {
                    try {
                        ...
                        osc.connect(gain); gain.connect(beepAudioCtx.destination);
                ...
                document.getElementById('btn-monitor-beep')?.addEventListener('click', () => {
                    if (!beepAudioCtx) beepAudioCtx = new (window.AudioContext || window.webkitAudioContext)();
                    beepEnabled = !beepEnabled;
                    if (beepEnabled) markExistingQRSBeeped();
                    const btn = document.getElementById('btn-monitor-beep');
